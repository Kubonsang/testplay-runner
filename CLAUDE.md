...
**Current released version:** `v0.12.0` (released 2026-08-03), Native CoW Storage Foundation. The public six-command `testplay` CLI contract and production default backend are unchanged. Release archives include the standalone experimental `testplay-storage-helper` v2 with schema 1 and native providers for Windows Differencing VHDX, macOS APFS clonefile, and Linux reflink. The helper is not wired to `testplay run` or the Image backend, is not selected automatically, has no silent physical-copy fallback, and is not production-ready. **Identity locked at v0.8.0:** contract layer for agents+CI, not a TDD speed tool — see Identity anchor above.
...
| `testplay run [--filter <name>] [--category <cat>] [--compare-run <run_id>] [--shadow] [--reset-shadow] [--clear-cache] [--workspace-backend legacy\|image] [--workspace-store-root <absolute-path>] [--keep-workspace] [--bridge] [--no-bridge] [--scenario <file>]` | Execute tests; streams progress to `testplay-status.json` (single mode) or `testplay-status-<role>.json` (scenario mode) |
...
**`--workspace-backend legacy|image` / `--workspace-store-root` / `--keep-workspace`**: Experimental Library Image spike. Omitted preserves the existing backend resolution. `legacy` explicitly forces the existing Shadow/cache path; `image` forces the immutable keyed image path and fails explicitly without Legacy fallback. `--workspace-store-root` moves only persistent cache/image data to a validated absolute directory outside the Unity project; Unity workspaces remain local. `--keep-workspace` preserves the test Shadow for debugging. These experimental options are unsupported in scenario mode. The measured verdict is `PROMISING`; see `docs/library-image-spike.md`.
...
- `.testplay/library-images/` — experimental immutable keyed Library Image store. Final images contain `Library/`, `metadata.json`, and a last-written `COMPLETE` marker; `locks/` serializes builder+commit per key and `quarantine/` retains corrupt images. Never selected unless `--workspace-backend=image` is explicit.
