...
testplay run --reset-shadow        # 섀도우 Library 캐시를 삭제·재구축 후 실행
```
...
Unity 에디터가 프로젝트를 열고 있으면 `Temp/UnityLockfile`이 존재하며, Unity 배치 모드가 동일한 프로젝트 디렉터리에서 실행될 수 없습니다. `testplay run`은 이를 자동으로 감지하고 프로젝트 루트 내 `.testplay-shadow/`에 섀도우 워크스페이스를 생성합니다:
...
**섀도우 모드는 에이전트에게 투명합니다.** JSON 출력의 모든 `absolute_path` 필드는 원본 프로젝트 경로로 재매핑됩니다 — 에이전트는 `.testplay-shadow/` 경로를 볼 수 없습니다.

**플래그:**
- `--shadow` — 에디터가 열려 있지 않아도 강제로 섀도우 워크스페이스를 사용 (섀도우 동작 테스트에 유용)
- `--reset-shadow` — 실행 전 `.testplay-shadow/`를 삭제하고 처음부터 재구축 (Unity 버전 업그레이드 후 또는 Library 캐시가 오래된 경우 사용)

**`.gitignore`는 최초 사용 시 자동으로 패치**되어 `.testplay-shadow/`가 제외됩니다.

**`--reset-shadow` 사용 시점:** Unity 버전이 업그레이드되었거나, 원본 프로젝트에서는 발생하지 않는 임포트 오류로 테스트가 실패하는 경우 Library 캐시가 오래된 것일 수 있습니다. `testplay run --reset-shadow`로 삭제 후 재구축하세요.
...
5. `--shadow` 플래그를 사용한 섀도우 모드 스모크 단계 실행 — `.testplay-shadow/` 워크스페이스 생성 및 예상 서브디렉터리 확인
