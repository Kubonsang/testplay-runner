...
| 6 | 빌드 실패 — **예약됨, 실제로 반환되지 않음.** 라이선스·빌드 타겟 오류는 현재 exit 1로 표시됨 | — |
| 7 | 권한 오류 — **예약됨, 실제로 반환되지 않음.** 권한 오류는 현재 exit 1 또는 exit 9로 표시됨 | — |
...
**폴링이 유일한 수단입니다.** 푸시 알림, 웹훅, SSE 엔드포인트는 없습니다. 에이전트는 일정 간격으로 `testplay-status.json`을 읽고 `updated_at`으로 stale 읽기를 감지해야 합니다.
...
**`testplay list`는 설계상 불완전합니다.**
정적 스캐너는 `[Test]`, `[UnityTest]`, `[TestCase]`, `[TestCaseSource]`, `[Theory]`만 감지합니다. 커스텀 어트리뷰트나 추상 기반 클래스 패턴을 사용하는 테스트는 보이지 않습니다. 출력 JSON에 완전성 여부(`complete: true/false`)를 알리는 신호가 없어, 결과만 보고는 목록이 완전한지 알 수 없습니다. 개선 방향: run 후 NUnit XML에서 전체 테스트 목록을 캐시해 이후 `list` 호출에서 Unity의 실제 인벤토리를 반환.

**Exit code 6과 7은 실제로 반환되지 않습니다.**
문서에 예약되어 있지만 러너가 실제로 반환하지 않습니다. Unity 라이선스 오류, 빌드 타겟 누락, Unity 바이너리 없음이 현재 모두 동일한 exit 1과 `hint` 필드로 나타납니다. 개선 방향: 라이선스·빌드 타겟 오류에 대한 stderr 패턴 매칭(exit 6), 파일 작업 전반에 `os.IsPermission` 체크(exit 7).

**진행 상황 모니터링은 폴링만 가능합니다.**
`testplay-status.json`이 실행 중 상태를 확인하는 유일한 채널입니다. SSE, 웹소켓, 네임드 파이프 없음. 너무 느리게 폴링하면 빠른 phase 전환을 놓치고, 너무 빠르면 불필요한 리소스를 소모합니다. `updated_at`으로 마지막 쓰기보다 이전 읽기를 감지하세요. 개선 방향: 변경 감지를 위한 `seq` 필드, PlayMode 네트워크 테스트 도입 시 선택적 SSE 엔드포인트.
